...
glm::vec3 barycentric_weights(glm::vec3 const &a, glm::vec3 const &b, glm::vec3 const &c, glm::vec3 const &pt) {
    ...
    float v = (d11 * d02 - d01 * d12) / d;
    float w = (d00 * d12 - d01 * d02) / d;
    float u = 1.0f - v - w;
...
void WalkMesh::walk_in_triangle(WalkPoint const &start, glm::vec3 const &step, WalkPoint *end_, float *time_) const {
	...
	glm::vec3 times = - start.weights / step_coords;
	float tmin = 1.0f;
	...
	time = tmin;
    glm::vec3 weights = start.weights + step_coords * tmin;
	...
	{
        end.weights = glm::vec3(
			weights.y / (weights.y + weights.z),
			weights.z / (weights.y + weights.z),
			0.0f
		);
		end.indices = glm::uvec3(
			start.indices.y,
			start.indices.z,
			start.indices.x
		);
	...
	else if (times.y != 1.0f && times.y == tmin) 
	{
			...
			weights.z / (weights.x + weights.z),
			weights.x / (weights.x + weights.z),
			0.0f
		);
		end.indices = glm::uvec3(
			start.indices.z,
...
bool WalkMesh::cross_edge(WalkPoint const &start, WalkPoint *end_, glm::quat *rotation_) const {
	...
	auto find_next_vertex = next_vertex.find(edge);
	if (find_next_vertex != next_vertex.end()) 
	{
		...
		end.indices.z = find_next_vertex->second;

    	rotation = glm::rotation(
			glm::normalize(glm::cross(
				vertices[start.indices.y] - vertices[start.indices.x], 
				vertices[start.indices.z] - vertices[start.indices.x])), 
			glm::normalize(glm::cross(
				vertices[end.indices.y] - vertices[end.indices.x], 
